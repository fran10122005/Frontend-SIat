...
import DiarioHogar from "./pages/DiarioHogar";
...
export default function App() {
  ...
  const renderPage = () => {
    ...
    if (safeView === "login" || safeView === "register")
      return <Auth currentView={safeView} onNavigate={navigate} />;
    ...
    if (safeView === "register-repre")
      return <RegisterRepre onNavigate={navigate} />;
    ...
    if (safeView === "patients")
      return <PatientManagement onNavigate={navigate} />;
    ...
    if (safeView === "historial")
      return <HistoryProgress onNavigate={navigate} />;
          ...
          <div className="bg-white dark:bg-slate-900 rounded-xl shadow-2xl w-full max-w-md overflow-hidden border border-blue-200 dark:border-blue-900/30 animate-in zoom-in-95 duration-200">
            <div className="p-6 text-center">
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400">
                Por políticas de seguridad clínica, tu sesión se cerrará
                automáticamente en 15 segundos si no hay actividad. ¿Deseas
                continuar?
              </p>
            </div>
            <div className="px-6 py-4 bg-gray-50 dark:bg-slate-800/50 border-t border-gray-100 dark:border-slate-800 flex justify-end gap-3">
              <button
                ...
                className="px-4 py-2 font-semibold text-sm bg-gray-200 dark:bg-slate-700 text-gray-800 dark:text-white hover:bg-gray-300 dark:hover:bg-slate-600 rounded-lg transition-colors"
              >
                Cerrar Sesión
              </button>
              <button
                onClick={handleContinueSession}
                className="px-4 py-2 font-semibold text-sm bg-brand-800 text-white hover:bg-brand-900 rounded-lg shadow-sm transition-colors"
              >
                Continuar Sesión
              </button>
            </div>
