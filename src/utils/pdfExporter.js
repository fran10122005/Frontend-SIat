...
const addDirectorSignature = (doc, y) => {
  const sigW = 80;
  const pageW = doc.internal.pageSize.width;
  const x = (pageW - sigW) / 2;

  doc.setFontSize(9);
  doc.setFont("Helvetica", "bold");
  doc.setTextColor(1, 28, 63);
  doc.text("Firma del Director", pageW / 2, y, { align: "center" });
...
export const createStyledPdf = async (
  title,
  filename,
  columns,
  rows,
  ...
  await addLogoToDoc(doc, pageW - 28, 6, 16, 16);
  ...
  autoTable(doc, {
    ...
    didDrawPage: (data) => {
      ...
      addLogoToDoc(doc, pageW - 28, 6, 16, 16);
      addSiatLogoToDoc(doc, 12, 4, 16, 16);
    },
  });

  if (showDirectorSignature) {
    const sigY = pageH - 30;
    if (doc.lastAutoTable.finalY > pageH - 40) {
      doc.addPage();
      await addLogoToDoc(doc, pageW - 28, 6, 16, 16);
      await addSiatLogoToDoc(doc, 12, 4, 16, 16);
      addDirectorSignature(doc, sigY);
      addFooter(doc, pageH - 10);
    } else {
      addDirectorSignature(doc, sigY);
    }
...
export const exportEspecialistasToPDF = async (especialistas) => {
  ...
  const rows = especialistas.map((esp) => {
    ...
    return [
      ...
      {
        content: esp.tm_usuar?.usu_estd !== false ? "Activo" : "Inactivo",
        styles: {
          textColor:
            esp.tm_usuar?.usu_estd !== false ? [16, 185, 129] : [239, 68, 68],
          fontStyle: "bold",
        },
      },
    ];
  ...
  await createStyledPdf(
    ...
    "especialistas_siat.pdf",
    columns,
    rows,
    { pageTitle: "Directorio de Especialistas — SIAT" },
  );
};
...
export const exportAsignacionesToPDF = async (asignaciones) => {
  ...
  const rows = asignaciones.map((asi) => [
    ...
    {
      content: asi.asi_stdo,
      styles: {
        textColor: asi.asi_stdo === "Activo" ? [16, 185, 129] : [100, 100, 100],
        fontStyle: "bold",
      },
    },
  ]);
  await createStyledPdf(
    ...
    "asignaciones_siat.pdf",
    columns,
    rows,
    { pageTitle: "Control de Casos y Asignaciones — SIAT" },
  );
};
...
export const exportEspecialidadesToPDF = async (especialidades) => {
  ...
  const rows = especialidades.map((esc) => [
    ...
    esc.esc_desc || "-",
    {
      content: esc.esc_estd !== false ? "Activa" : "Inactiva",
      styles: {
        textColor: esc.esc_estd !== false ? [16, 185, 129] : [239, 68, 68],
        fontStyle: "bold",
      },
    },
  ]);
  await createStyledPdf(
    ...
    "especialidades_siat.pdf",
    columns,
    rows,
    { pageTitle: "Catálogo de Especialidades — SIAT" },
  );
};

export const exportHistoryToPDF = async (
  historicalData,
  title = "Reporte de Evolución Médica",
) => {
  ...
  const rows = historicalData.map((row) => [
    ...
    {
      content: row.fue_efec ? "Efectiva" : "No Efectiva",
      styles: {
        textColor: row.fue_efec ? [16, 185, 129] : [239, 68, 68],
        fontStyle: "bold",
      },
    },
    row.com_tend || "Sin notas",
  ]);
  await createStyledPdf(title, "reporte_evolucion_siat.pdf", columns, rows, {
    pageTitle: `${title} — SIAT`,
  });
};
...
export const exportDashboardReport = async ({
  ...
  doc.text(
    `Rol: ${userRole === "ADMIN_INSTITUCION" ? "Admin Institucional" : userRole === "ESPECIALISTA" ? "Especialista" : "Representante"}`,
    margin + 90,
    infoY,
  );
  doc.text(
  ...
  if (fechaInicio && fechaFin)
    doc.text(`Período: ${fechaInicio} — ${fechaFin}`, margin + 90, infoY);
  y += paciente ? 30 : 24;
  ...
  if (alerts.length > 0) {
    ...
    const alertRows = showAlerts.map((a, i) => [
      (i + 1).toString(),
      a.time ||
        new Date(a.ale_time).toLocaleTimeString("es-ES", {
          hour: "2-digit",
          minute: "2-digit",
        }),
    ...
    if (alerts.length > 15) {
      ...
      doc.setTextColor(150, 150, 150);
      doc.text(
        `* Mostrando 15 de ${alerts.length} alertas registradas.`,
        margin,
        y,
      );
      y += 5;
  ...
  const notasTxt =
    "Este reporte fue generado automáticamente por SIAT. Los datos aquí presentados reflejan el estado del sistema al momento de la exportación. Para mayor detalle, consulte el panel en línea.";
  ...
  doc.text(
    `ID de exportación: EXP-${Date.now().toString(36).toUpperCase()}`,
    margin,
    y + 3,
  );
  doc.text(
    `© ${new Date().getFullYear()} SIAT — Funauta. Todos los derechos reservados.`,
    margin,
    y + 6,
  );
