...
export default function HardwareInventory() {
  ...
  const { telemetryHistory, isWebSocketActive, simulationMode, setSimulationMode } = useTelemetry()
  ...
  const handleSimulationToggle = (estado) => {
    if (isWebSocketActive) {
      api.post('/monitoreo/simular-estado', { estado })
        .then(() => {
          showToast(
            estado === 'CRISIS'
              ...
              <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-5">
                  ...
                  <div className="flex items-center gap-3 flex-wrap">
                    <span className={`px-3 py-1 text-xs font-bold rounded-full border flex items-center gap-1.5 ${isWebSocketActive
                      ? 'bg-blue-500/10 border-blue-500/20 text-blue-600 dark:text-blue-400'
                      : 'bg-amber-500/10 border-amber-500/20 text-amber-600 dark:text-amber-400'
                      }`}>
                    </span>
                    <span className={`px-3 py-1 text-xs font-bold rounded-full border flex items-center gap-1.5 ${isOnline
                    ...
                    >
                      <Activity className="w-3.5 h-3.5" /> {isSimulatingCrisis ? 'Detener Crisis' : 'Crisis'}
                    </button>
                    <button
                    ...
                    >
                      <Activity className="w-3.5 h-3.5" /> {isSimulatingCalma ? 'Detener Calma' : 'Calma'}
                    </button>
                  </div>
                </div>
                    ...
                    <div className="bg-white dark:bg-slate-800 rounded-xl p-6 shadow-sm border border-slate-200 dark:border-slate-700/60 flex flex-col gap-4 relative overflow-hidden">
                      ...
                      <div className="space-y-2">
                        <div className="w-full bg-slate-100 dark:bg-slate-700 rounded-full h-2.5 overflow-hidden">
                          <div
                            className={`h-2.5 rounded-full transition-all duration-500 ${currentStress > 75 ? 'bg-rose-500' : currentStress > 40 ? 'bg-amber-500' : 'bg-emerald-500'
                              }`}
