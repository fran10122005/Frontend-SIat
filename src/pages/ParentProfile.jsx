...
export default function ParentProfile() {
  const { userRole, navigate, showToast, selectedChildId, globalPeiGoals } =
    useGlobalContext();
          ...
          >
            <div className="flex gap-6 overflow-x-auto no-scrollbar">
              ...
              <button
                ...
                className={`py-3 text-sm font-medium transition-colors border-b-2 whitespace-nowrap ${activeTab === "metas" ? "border-brand-500 text-brand-500" : "border-transparent text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200"}`}
              >
                <span className="flex items-center gap-2">
                ...
                <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
                  <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-700 flex items-center gap-2">
                    ...
                    <h3 className="font-bold text-slate-800 dark:text-white text-sm">
                      Metas del Plan Educativo Individualizado (PEI)
                    </h3>
                    <span className="ml-auto text-xs font-semibold text-slate-400">
                      {(globalPeiGoals || []).length} metas activas
                    </span>
                  </div>

                  {!globalPeiGoals || globalPeiGoals.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-16 gap-3 text-slate-400">
                      <Target className="w-10 h-10 opacity-30" />
                      <p className="text-sm font-medium">
                        No hay metas PEI registradas aún
                      </p>
                      <p className="text-xs text-slate-400 text-center max-w-xs">
                        El especialista definirá las metas terapéuticas del plan
                        de tu niño.
                      </p>
                    </div>
                  ) : (
                        ...
                        return (
                          ...
                          >
                            <div className="flex items-start justify-between gap-2 mb-3">
                              <p className="text-sm font-semibold text-slate-800 dark:text-white leading-snug flex-1">
                                {g.met_desc}
                              </p>
                              {isComplete ? (
                                <span className="shrink-0 flex items-center gap-1 text-[11px] font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-100 dark:bg-emerald-900/30 px-2 py-0.5 rounded-full">
                                  <CheckCircle2 className="w-3.5 h-3.5" />{" "}
                                  Lograda
                                </span>
                              ) : (
                                <span className="shrink-0 text-[11px] font-bold text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-900/30 px-2 py-0.5 rounded-full">
                                  En Progreso
                                </span>
                              )}
                            </div>
                            ...
                            <div className="flex items-center justify-between text-xs text-slate-500 dark:text-slate-400 mb-1.5">
                              ...
                              <span className="font-bold tabular-nums text-slate-700 dark:text-slate-300">
                                {g.met_trial || 0} / {g.met_ttria || 0} ensayos
                              </span>
                            </div>
