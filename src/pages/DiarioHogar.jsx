...
const mockBitacoras = [
  {
    bit_codi: 0,
    bit_fech: new Date().toISOString().split("T")[0],
    bit_anim: "Estable",
    bit_crisi: 0,
    bit_suen: 8,
    bit_apet: "Bueno",
    bit_dese: "",
    bit_obse: "Sin novedades aún.",
  },
  {
    bit_codi: 1,
    bit_fech: new Date(Date.now() - 86400000).toISOString().split("T")[0],
    bit_anim: "Estable",
    bit_crisi: 0,
    bit_suen: 8,
    bit_apet: "Bueno",
    bit_dese: "Ruido de aspiradora",
    bit_obse: "Buena jornada, completó todas las rutinas.",
  },
  {
    bit_codi: 2,
    bit_fech: new Date(Date.now() - 172800000).toISOString().split("T")[0],
    bit_anim: "Irritable",
    bit_crisi: 2,
    bit_suen: 6,
    bit_apet: "Regular",
    bit_dese: "Cambio en la rutina de la tarde",
    bit_obse: "Dos crisis breves, se recuperó con respiración.",
  },
  {
    bit_codi: 3,
    bit_fech: new Date(Date.now() - 259200000).toISOString().split("T")[0],
    bit_anim: "Muy Calmo",
    bit_crisi: 0,
    bit_suen: 9,
    bit_apet: "Bueno",
    bit_dese: "",
    bit_obse: "Sin novedades.",
  },
  {
    bit_codi: 4,
    bit_fech: new Date(Date.now() - 345600000).toISOString().split("T")[0],
    bit_anim: "Estable",
    bit_crisi: 0,
    bit_suen: 7,
    bit_apet: "Selectivo",
    bit_dese: "Textura de alimentos nuevos",
    bit_obse: "Rechazó la cena.",
  },
];
...
export default function DiarioHogar() {
  ...
  const filteredBitacoras = useMemo(() => {
    ...
    if (filterDateFrom)
      data = data.filter(
        (b) => new Date(b.bit_fech) >= new Date(filterDateFrom),
      );
    if (filterDateTo)
      data = data.filter(
        (b) => new Date(b.bit_fech) <= new Date(filterDateTo + "T23:59:59"),
      );
    if (filterMood !== "TODOS")
  ...
  useEffect(() => {
    setPage(0);
  }, [filterDateFrom, filterDateTo, filterMood, filterCrisisOnly]);
  ...
  const totalCrisis = bitacorasList.reduce(
    (sum, b) => sum + (b.bit_crisi || 0),
    0,
  );
                ...
                >
                  ...
                  <label className="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-400 cursor-pointer select-none">
                    <input
                      ...
                      onChange={(e) => setFilterCrisisOnly(e.target.checked)}
                      className="rounded border-gray-300 dark:border-slate-600 text-brand-500 focus:ring-brand-500"
                    />
                    Solo crisis
                  </label>
                  {hasFilters && (
