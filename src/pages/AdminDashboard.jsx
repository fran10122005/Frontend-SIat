...
import {
  exportEspecialistasToPDF as expEspPDF,
  exportAsignacionesToPDF as expAsigPDF,
  exportEspecialidadesToPDF as expCatPDF,
...
import HistorialClinicoTab from "../components/admin/HistorialClinicoTab";
...
function AdminDashboard({ onNavigate }) {
  ...
  const handleInviteSubmit = async (e) => {
    ...
    try {
      ...
      showToast(
        "✅ Invitación clínica de niño y representante creada con éxito",
      );
      setFormData({
        nin_nomb: "",
        nin_apel: "",
  ...
  const handleToggleActivo = (esp_codi, currentState) => {
    setModalConfig({
      ...
      onConfirm: async () => {
        try {
          await api.patch(`/admin/especialistas/${esp_codi}/estado`, {
            activo: !currentState,
          });
          showToast(
            `✅ Especialista ${!currentState ? "activado" : "desactivado"} exitosamente.`,
          );
          fetchData();
        } catch (err) {
          showToast(
  ...
  const handleToggleAsignacion = (asi_codi, currentState) => {
    ...
    setModalConfig({
      ...
      onConfirm: async () => {
        try {
          await api.patch(`/admin/asignaciones/${asi_codi}/estado`, {
            estado: nextState,
          });
  ...
  const handleUpdateEspecialidad = async (e) => {
    ...
    try {
      await api.put(
        `/admin/especialidades/${editingEspCat.esc_codi}`,
        editingEspCat,
      );
  ...
  const handleToggleEspecialidad = (esc_codi, currentState) => {
    ...
    setModalConfig({
      ...
      onConfirm: async () => {
        try {
          await api.patch(`/admin/especialidades/${esc_codi}/estado`, {
            activo: !nextState,
          });
          showToast(
            `✅ Especialidad ${!nextState ? "activada" : "desactivada"} exitosamente.`,
          );
          fetchData();
        } catch (err) {
          showToast(
  ...
  const exportAsignacionesToExcel = () => {
    const data = asignaciones.map((asi) => ({
      ...
      Paciente:
        `${asi.tm_ninos?.nin_nomb || ""} ${asi.tm_ninos?.nin_apel || ""}`.trim() ||
        "Desconocido",
      "ID Especialista": asi.tm_espec?.esp_codi || "N/A",
      Especialista:
        `${asi.tm_espec?.esp_gner === "M" ? "Dr." : "Dra."} ${asi.tm_espec?.esp_nomb || ""} ${asi.tm_espec?.esp_apel || ""}`.trim() ||
        "Desconocido",
  ...
  const exportUsuariosToPDF = async () => {
    ...
    const rows = usuarios.map((u) => [
      ...
      {
        content: u.usu_estd !== false ? "Activo" : "Inactivo",
        styles: {
          textColor: u.usu_estd !== false ? [16, 185, 129] : [239, 68, 68],
          fontStyle: "bold",
        },
      },
    ]);
    await createStyledPdf(
      ...
      columns,
      rows,
      { pageTitle: "Usuarios del Sistema — SIAT" },
    );
  };
  ...
  const exportUsuariosToExcel = () => {
    ...
    const data = usuarios.map((u) => ({
      ...
      Creación: u.usu_crea
        ? new Date(u.usu_crea).toLocaleDateString("es-ES")
        : "-",
      "Último Acceso": u.usu_logi
        ? new Date(u.usu_logi).toLocaleDateString("es-ES")
        : "Nunca",
    ...
    <div className="flex h-[100dvh] w-full bg-[#F8FAFC] dark:bg-[#0B1120] font-sans overflow-hidden transition-colors duration-200">
      ...
      <AdminSidebar
        ...
        counts={{
          ...
          incidentes: incidentes.length || 5,
          asignaciones: asignaciones.filter((a) => a.asi_stdo === "Activo")
            .length,
              ...
              <div className="flex flex-col gap-2">
                <h1 className="text-xl md:text-2xl font-bold text-brand-700 dark:text-blue-400 tracking-tight flex items-center gap-2 md:gap-3 transition-colors">
                  ...
                  {activeTab === "asignaciones" &&
                    "Control de Casos y Asignaciones"}
              ...
              {activeTab === "dashboard" && (
                <div className="flex gap-2">
                    ...
                    onClick={async () => {
                      try {
                        ...
                        const alertsFromLogs = auditLogs
                          .filter(
                            (log) =>
                              log.aud_tipo === "INCIDENTE" ||
                              log.aud_tipo === "WARN",
                          )
                          .slice(0, 20)
                          .map((log) => ({
                            time: new Date(log.aud_time).toLocaleTimeString(
                              "es-ES",
                        ...
                        await exportDashboardReport({
                          ...
                          kpis: [
                            {
                              label: "Especialistas",
                              value: especialistas.length.toString(),
                            },
                            {
                              label: "Pacientes",
                              value: ninos.length.toString(),
                            },
                            {
                              label: "Asignaciones Activas",
                              value: asignaciones
                                .filter((a) => a.asi_stdo === "Activo")
                                .length.toString(),
                            },
                            {
                  ...
                  <button
                    ...
                    className="px-4 py-2.5 bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 font-semibold rounded-lg shadow-sm border border-blue-200 dark:border-blue-800/50 hover:bg-blue-100 dark:hover:bg-blue-900/50 transition-all flex items-center gap-2 text-sm shrink-0"
                  >
          ...
          <div className="bg-white dark:bg-[#1E293B] rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden border border-slate-200 dark:border-slate-800 animate-in zoom-in-95 duration-200">
            <div className="px-6 py-4 bg-[#034EA1] text-white flex items-center justify-between">
              <h3 className="text-lg font-bold">
                Registrar Niño e Invitar Representante
              </h3>
              <button
            ...
            >
              <h4 className="text-xs font-bold uppercase tracking-wider text-indigo-600 dark:text-indigo-400 border-b pb-1">
                Información del Paciente (Niño)
              </h4>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    required
                    type="text"
                    value={formData.nin_nomb}
                    onChange={(e) =>
                      setFormData({ ...formData, nin_nomb: e.target.value })
                    }
                    className="w-full px-3 py-2 text-sm bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg outline-none focus:ring-2 focus:ring-blue-500"
                ...
                <div>
                  ...
                  <input
                    required
                    type="text"
                    value={formData.nin_apel}
                    onChange={(e) =>
                      setFormData({ ...formData, nin_apel: e.target.value })
                    }
                    className="w-full px-3 py-2 text-sm bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg outline-none focus:ring-2 focus:ring-blue-500"
              ...
              <div className="grid grid-cols-3 gap-3">
                <div>
                  ...
                  <input
                    required
                    type="date"
                    value={formData.nin_fnac}
                    onChange={(e) =>
                      setFormData({ ...formData, nin_fnac: e.target.value })
                    }
                    className="w-full px-2 py-2 text-xs bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg outline-none focus:ring-2 focus:ring-blue-500"
                  />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1">
                  ...
                  <select
                    value={formData.nin_gner}
                    onChange={(e) =>
                      setFormData({ ...formData, nin_gner: e.target.value })
                    }
                    className="w-full px-2 py-2 text-xs bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg outline-none focus:ring-2 focus:ring-blue-500 cursor-pointer"
                  >
                ...
                <div>
                  ...
                  <select
                    value={formData.nin_nivd}
                    onChange={(e) =>
                      setFormData({ ...formData, nin_nivd: e.target.value })
                    }
                    className="w-full px-2 py-2 text-xs bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg outline-none focus:ring-2 focus:ring-blue-500 cursor-pointer"
                  >
              ...
              <h4 className="text-xs font-bold uppercase tracking-wider text-indigo-600 dark:text-indigo-400 border-b pb-1 pt-2">
                Información de la Invitación (Representante)
              </h4>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    required
                    type="text"
                    value={formData.rep_nomb}
                    onChange={(e) =>
                      setFormData({ ...formData, rep_nomb: e.target.value })
                    }
                    className="w-full px-3 py-2 text-sm bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg outline-none focus:ring-2 focus:ring-blue-500"
                ...
                <div>
                  ...
                  <input
                    required
                    type="text"
                    value={formData.rep_apel}
                    onChange={(e) =>
                      setFormData({ ...formData, rep_apel: e.target.value })
                    }
                    className="w-full px-3 py-2 text-sm bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg outline-none focus:ring-2 focus:ring-blue-500"
                    placeholder="Ej. Pérez"
                  />
              ...
              <div>
                ...
                <input
                  required
                  type="email"
                  value={formData.usu_crro}
                  onChange={(e) =>
                    setFormData({ ...formData, usu_crro: e.target.value })
                  }
                  className="w-full px-3 py-2 text-sm bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg outline-none focus:ring-2 focus:ring-blue-500"
                  placeholder="correo@repre.com"
                />
                <p className="text-[10px] text-slate-400 mt-1">
                  El sistema pre-registrará al representante y le enviará un
                  enlace de activación por correo.
                </p>
              </div>
              ...
              <div className="flex justify-end gap-3 pt-3 border-t">
                <button
                  type="button"
                  onClick={() => setShowRegModal(false)}
                  className="px-4 py-2 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 rounded-lg text-sm font-semibold hover:bg-slate-200"
                >
                  Cancelar
                </button>
                <button
