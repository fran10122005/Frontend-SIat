...
import { 
  ...
  CheckCircle2, Plus, Star, MessageSquare,
...
export default function Routines() {
  const { nomNino, showToast, routines, createRoutine, selectedChildId, globalPeiGoals = [], crearPeiGoal } = useGlobalContext()
                          ...
                          <div className="space-y-4">
                            ...
                            {formData.mediaType === 'video' && (
                              <div className="mt-4 relative animate-in zoom-in-95">
                                ...
                                <input type="url" placeholder="https://youtube.com/watch?v=..." value={formData.mediaUrl} onChange={e => setFormData({...formData, mediaUrl: e.target.value})} className="w-full pl-5 pr-12 py-4 bg-slate-50 dark:bg-[#1E293B] border border-slate-200 dark:border-slate-700 rounded-2xl outline-none focus:border-rose-500 focus:ring-4 focus:ring-rose-500/20 text-sm font-medium text-slate-900 dark:text-white transition-all" />
                              </div>
                            )}
