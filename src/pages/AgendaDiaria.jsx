...
export default function AgendaDiaria() {
  ...
  const saveSession = async () => {
    ...
    if (
      activeSession.title.toLowerCase().includes("dientes") ||
      activeSession.title.toLowerCase().includes("higiene")
    ) {
      setAgenda((prev) =>
        prev.map((t) => (t.id === 1 ? { ...t, done: true } : t)),
      );
    } else if (
      activeSession.title.toLowerCase().includes("sensorial") ||
      activeSession.title.toLowerCase().includes("integración")
    ) {
      setAgenda((prev) =>
        prev.map((t) => (t.id === 3 ? { ...t, done: true } : t)),
      );
    } else if (
      activeSession.title.toLowerCase().includes("comer") ||
      activeSession.title.toLowerCase().includes("alimentación")
    ) {
      setAgenda((prev) =>
        prev.map((t) => (t.id === 2 ? { ...t, done: true } : t)),
      );
    }
