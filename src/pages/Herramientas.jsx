...
const sensoryStrategies = [
  {
    id: "deep_pressure",
    label: "Presión Profunda",
    emoji: "🫂",
    desc: "Abrazo firme o peso en los hombros que ayuda a calmar el sistema nervioso.",
  },
  {
    id: "breathing",
    label: "Respiración",
    emoji: "🌬️",
    desc: "Inhalar 4s, sostener 4s, exhalar 4s. Repite 3 veces.",
  },
  {
    id: "swing",
    label: "Balanceo",
    emoji: "🪑",
    desc: "Movimiento rítmico suave que organiza el sistema vestibular.",
  },
  {
    id: "weighted",
    label: "Manta con Peso",
    emoji: "🛏️",
    desc: "Presión profunda distribuida que genera sensación de seguridad.",
  },
  {
    id: "headphones",
    label: "Audífonos",
    emoji: "🎧",
    desc: "Reducción de ruido ambiental para evitar sobrecarga sensorial auditiva.",
  },
  {
    id: "water",
    label: "Agua Fría",
    emoji: "💧",
    desc: "Beber o mojar las manos con agua fría para resetear el sistema.",
  },
  {
    id: "jump",
    label: "Saltar",
    emoji: "🦘",
    desc: "10 saltos para liberar energía y regular el sistema propioceptivo.",
  },
  {
    id: "squeeze",
    label: "Apretar",
    emoji: "🧸",
    desc: "Pelota antiestrés o masilla — presión en manos que organiza.",
  },
  {
    id: "massage",
    label: "Masaje",
    emoji: "💆",
    desc: "Presión firme en brazos, manos o espalda para relajar.",
  },
  {
    id: "dim_lights",
    label: "Luz Tenue",
    emoji: "🌙",
    desc: "Reducir la luz ambiental para disminuir estímulos visuales.",
  },
  {
    id: "rocking",
    label: "Mecedora",
    emoji: "🪑",
    desc: "Movimiento suave y repetitivo que calma y organiza.",
  },
  {
    id: "chew",
    label: "Masticar",
    emoji: "🫦",
    desc: "Morder un mordedor o snack crujiente — entrada oral propioceptiva.",
  },
];
...
export default function Herramientas() {
  ...
  const [tasks, setTasks] = useState(() => {
    ...
    return saved
      ...
      : [
          ...
          {
            id: 3,
            title: "Terminar la tarea escolar",
            reward: 2,
            completed: false,
            emoji: "📚",
          },
          {
            id: 4,
            title: "Seguir indicaciones a la primera",
            reward: 1,
            completed: false,
            emoji: "👂",
          },
  ...
  useEffect(() => {
    localStorage.setItem(
      "siat_custom_pictograms",
      JSON.stringify(customPictograms),
    );
  }, [customPictograms]);
  ...
  useEffect(() => {
    localStorage.setItem(
      "siat_earning_history",
      JSON.stringify(earningHistory),
    );
  }, [earningHistory]);
  ...
  const startTimer = useCallback(() => {
    ...
    if (total <= 0) {
      showToast("⚠️ Pon un tiempo mayor a 0");
      return;
    }
  ...
  const redeemReward = (reward) => {
    if (balance >= reward.cost) {
      ...
      setHistory([
        { title: reward.title, date: timeStr, emoji: reward.emoji },
        ...history,
      ]);
  ...
  const handleAddReward = (e) => {
    ...
    if (isNaN(costInt) || costInt <= 0) {
      showToast("⚠️ Costo debe ser positivo");
      return;
    }
    setRewards((prev) => [
      ...prev,
        ...
        if (nextState) {
          ...
          setEarningHistory((prev) => {
            ...
            if (idx >= 0)
              existing[idx] = {
                ...existing[idx],
                count: existing[idx].count + t.reward,
              };
            else existing.push({ date: today, count: t.reward });
            return existing;
          });
          showToast(
            `⭐ +${t.reward} ${t.reward === 1 ? "estrella" : "estrellas"}`,
          );
        } else {
  ...
  const tabs = [
    {
      id: "aac",
      label: "Comunicación (AAC)",
      icon: MessageSquare,
      color: "bg-blue-600",
    },
    {
      id: "firstthen",
      label: "Primero-Después",
      icon: ArrowRight,
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="max-w-[1400px] w-full mx-auto p-4 md:p-6 flex flex-col gap-4 pb-8">
            ...
            <div className="flex flex-col gap-3">
              ...
              <div
                data-tour="ht-tabs"
                className="flex gap-1.5 overflow-x-auto pb-1 flex-nowrap -mx-1 px-1 scrollbar-none"
              >
                {tabs.map((tab) => {
                  const Icon = tab.icon;
              ...
              <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 flex flex-col gap-4">
                    ...
                    <div className="flex-1 min-h-[60px] bg-slate-50/50 dark:bg-slate-900/50 rounded-xl border border-slate-200 dark:border-slate-700 p-3 flex flex-wrap gap-2 items-center">
                      {sentence.length === 0 ? (
                        <span className="text-slate-400 dark:text-slate-500 font-bold uppercase tracking-widest text-xs flex items-center gap-2">
                          ...
                          Toca los pictogramas para construir una frase
                        </span>
                      ) : (
                        sentence.map((item, idx) => (
                          <div
                            key={idx}
                            className="bg-white dark:bg-slate-800 px-3 py-1.5 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 flex items-center gap-2 animate-in zoom-in-95"
                          >
                ...
                <div className="flex flex-wrap gap-3 items-center">
                  <div className="relative w-full sm:flex-1 sm:min-w-[200px]">
                    ...
                    <input
                      type="text"
                      data-tour="ht-aac-search"
                      placeholder="Buscar pictograma..."
                      value={searchQuery}
                      onChange={(e) => setSearchQuery(e.target.value)}
                      className="w-full pl-3 pr-9 py-2 text-xs bg-white dark:bg-[#1E293B] border border-slate-200 focus:border-brand-500 dark:border-slate-800 focus:ring-2 focus:ring-brand-500/15 rounded-xl outline-none text-slate-700 dark:text-slate-300 shadow-sm"
                    />
                  </div>
                  {searchQuery && (
                    <button
                      onClick={() => setSearchQuery("")}
                      className="px-2.5 py-2 text-xs font-semibold text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg transition-colors shrink-0"
                    >
                      Limpiar
                    </button>
                  )}
                ...
                {showAddPicForm && (
                  <form
                    onSubmit={handleAddPictogram}
                    className="bg-white dark:bg-[#1E293B] p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm space-y-3 animate-in slide-in-from-top-2 duration-200"
                  >
                ...
                <div
                  data-tour="ht-aac-grid"
                  className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4"
                >
                  {aacCategories.map((cat) => {
                    const customItems = customPictograms.filter(
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="grid grid-cols-2 gap-2 flex-1">
                          {filtered.map((item, i) => {
                            const isCustom = customPictograms.some(
                              (cp) => cp.label === item.label,
                            );
                            return (
                              <div key={i} className="relative group/item">
                                ...
                                {isCustom && (
                                  <button
                                    onClick={() =>
                                      handleDeleteCustomPictogram(item.label)
                                    }
            ...
            {activeTool === "firstthen" && (
              <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 flex flex-col gap-4">
                <div
                  data-tour="ht-ft-panel"
                  className="bg-white dark:bg-[#1E293B] rounded-2xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm"
                >
                  ...
                  {ftCompleted ? (
                    <div className="mt-6 flex justify-center">
                      <button
                        onClick={handleFtReset}
                        className="px-6 py-2.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 font-bold text-sm rounded-xl transition-colors flex items-center gap-2"
                      >
                    ...
                    <div className="mt-6 flex flex-col items-center gap-3">
                        ...
                        <select
                          ...
                          onChange={(e) => {
                            ...
                            if (found) {
                              ...
                              setFtCustomTask(e.target.value);
                              setFtTask(null);
                            }
                          }}
                        ...
                        <select
                          ...
                          onChange={(e) => {
                            ...
                            if (found) {
                              ...
                              setFtCustomReward(e.target.value);
                              setFtReward(null);
                            }
                          }}
                          className="px-3 py-2 text-xs bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg outline-none focus:ring-2 focus:ring-emerald-500 cursor-pointer max-w-[200px]"
                      ...
                      <button
                        data-tour="ht-ft-complete"
                        onClick={handleFtComplete}
                ...
                <div className="bg-amber-50 dark:bg-amber-900/20 rounded-xl p-4 border border-amber-200 dark:border-amber-900/50">
                  <p className="text-xs text-amber-800 dark:text-amber-300 font-medium flex items-start gap-2">
                    ...
                    <span>
                      ...
                      reduce la ansiedad por transiciones y le da
                      predictibilidad. Siempre cumple con el premio al terminar.
                    </span>
                  </p>
                </div>
              </div>
            ...
            {activeTool === "timer" && (
              <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 flex flex-col gap-4">
                <div
                  data-tour="ht-timer-panel"
                  className="bg-white dark:bg-[#1E293B] rounded-2xl p-6 md:p-8 border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col items-center gap-6"
                >
                  {/* Circular Timer */}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex gap-2">
                      {!timerRunning ? (
                        <button
                          onClick={startTimer}
                          className="px-5 py-2.5 bg-emerald-500 hover:bg-emerald-600 text-white font-bold text-sm rounded-xl shadow-md transition-all flex items-center gap-2"
                        >
                        ...
                        <>
                          <button
                            onClick={pauseTimer}
                            className={`px-5 py-2.5 font-bold text-sm rounded-xl shadow-md transition-all flex items-center gap-2 ${timerPaused ? "bg-green-500 hover:bg-green-600 text-white" : "bg-amber-500 hover:bg-amber-600 text-white"}`}
                          >
                            {timerPaused ? (
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400 text-center">
                    El temporizador cambia de color a medida que se acaba el
                    tiempo. Útil para anticipar transiciones.
                  </p>
                </div>
              </div>
            )}
              ...
              >
                <div className="flex items-center gap-3 bg-blue-50 dark:bg-blue-900/20 p-3 rounded-xl border border-blue-200 dark:border-blue-900/50">
                  ...
                  <p className="text-xs text-blue-800 dark:text-blue-300 font-medium">
                    ...
                    El catálogo de recompensas ha sido configurado por el
                    especialista para respetar la dieta y sensibilidad del
                    paciente.
                  </p>
                </div>
                  ...
                  <div className="flex items-end gap-2 h-24">
                    {weekChart.days.map((day, i) => (
                      ...
                      >
                        ...
                        >
                          <div
                            className="absolute bottom-0 w-full bg-amber-400 dark:bg-amber-500 rounded-t-md transition-all duration-500"
                            style={{
                              height: `${(day.count / weekChart.maxVal) * 100}%`,
                            }}
                          ></div>
                        </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
                  ...
                  <div className="lg:col-span-4 flex flex-col gap-4">
                    <div
                      data-tour="ht-econ-balance"
                      className="bg-white dark:bg-[#1E293B] rounded-2xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col items-center text-center justify-center min-h-[180px] lg:min-h-[250px]"
                    >
                    ...
                    <div className="bg-white dark:bg-[#1E293B] rounded-2xl p-4 border border-slate-200 dark:border-slate-800 shadow-sm flex-1">
                      ...
                      {history.length === 0 ? (
                        <p className="text-[11px] text-slate-400 dark:text-slate-500">
                          No hay premios canjeados recientemente.
                        </p>
                      ) : (
                        <div className="space-y-2 max-h-[160px] overflow-y-auto pr-1">
                          {history.map((h, idx) => (
                            <div
                              key={idx}
                              className="flex justify-between items-center p-2 bg-slate-50 dark:bg-slate-900/50 rounded-lg border border-slate-100 dark:border-slate-800"
                            >
                    ...
                    <div className="bg-white dark:bg-[#1E293B] rounded-2xl p-4 border border-slate-200 dark:border-slate-800 shadow-sm flex-1 flex flex-col min-h-[250px] lg:min-h-[400px]">
                      ...
                      <div className="space-y-2.5 overflow-y-auto max-h-[300px] flex-1 pr-1">
                        {tasks.map((t) => (
                          <div
                            key={t.id}
                            className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-xl transition-all hover:border-slate-300 dark:hover:border-slate-700 group/task"
                          >
