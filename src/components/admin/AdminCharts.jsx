    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="mb-6 flex justify-between items-start">
          <div>
            <h2 className="text-sm font-bold text-slate-900 dark:text-white uppercase tracking-wide">
              Carga Clínica y Pacientes
            </h2>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
              Evolución de admisiones e ingresos de pacientes
            </p>
          </div>
          {activityData.length > 0 && (
            <span className="text-[11px] font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/30 px-2.5 py-1 rounded-full whitespace-nowrap">
              {activityData.length} meses registrados
            </span>
          )}
        </div>
          ...
          {activityData.length > 0 ? (
            ...
            <div className="w-full h-full flex flex-col items-center justify-center text-slate-400 dark:text-slate-500 py-12">
              <p className="text-sm font-semibold">
                Sin datos de admisiones históricas
              </p>
            </div>
          )}
        </div>
      ...
      <div className="bg-white dark:bg-[#1E293B] rounded-xl p-6 shadow-sm border border-slate-200 dark:border-slate-800/60 flex flex-col min-h-[300px] lg:h-[400px] transition-all duration-200 hover:shadow-md">
        <div className="mb-6 flex justify-between items-start">
          <div>
            <h2 className="text-sm font-bold text-slate-900 dark:text-white uppercase tracking-wide">
              Productividad Terapéutica
            </h2>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
              Horas de terapia impartidas mensualmente
            </p>
          </div>
          {activityData.length > 0 && (
            <span className="text-[11px] font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/30 px-2.5 py-1 rounded-full whitespace-nowrap">
              {activityData.reduce((s, d) => s + (d.horasTerapia || 0), 0)}h
              total
            </span>
          )}
        </div>
          ...
          {activityData.length > 0 ? (
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                />
                <Bar
                  dataKey="horasTerapia"
                  fill="url(#barGrad)"
                  radius={[6, 6, 0, 0]}
                  maxBarSize={42}
                  name="Horas de Terapia"
                />
              </BarChart>
            </ResponsiveContainer>
          ) : (
            <div className="w-full h-full flex flex-col items-center justify-center text-slate-400 dark:text-slate-500 py-12">
              <p className="text-sm font-semibold">
                Sin registros de horas de terapia
              </p>
            </div>
          )}
        </div>
