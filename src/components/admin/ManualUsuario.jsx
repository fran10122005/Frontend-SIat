...
import {
  ...
  Link2, Users, ShieldCheck, Server, Settings, HelpCircle, BookMarked,
  ChevronRight, Printer, Maximize2, Minimize2, Sun, CheckCircle, AlertTriangle,
  Info, ArrowRight, GraduationCap
} from 'lucide-react';
...
const secciones = [
  {
    ...
    contenido: [
      { tipo: 'texto', valor: 'El módulo de Administrador está diseñado para el perfil ADMIN_INSTITUCION (Administrador de una fundación o clínica). Desde este panel se gestionan especialistas, pacientes, asignaciones, catálogos, cuentas de usuario, auditoría y el monitoreo de infraestructura del sistema.' }
    ]
  },
  {
    id: 'estructura',
    ...
    contenido: [
      ...
      { tipo: 'tabla', encabezados: ['Sección', 'Ícono', 'Descripción'],
        filas: [
          ['Panel Principal', '📊', 'KPIs, gráficos y bitácora de actividad'],
          ...
          ['Auditoría', '🛡️', 'Registro histórico de eventos de seguridad'],
  ...
  },
  {
    ...
    contenido: [
      ...
      { tipo: 'subtitulo', valor: 'Bitácora de Operaciones (Logs)' },
      { tipo: 'texto', valor: 'Tabla con el registro de actividades clínicas recientes: timestamp, tipo de evento (INFO, WARN, SUCCESS, INCIDENTE, ASIGNACION) y detalle del evento.' }
    ]
  },
  {
    id: 'especialistas',
  ...
  },
  {
    ...
    titulo: 'Auditoría',
    icono: 'auditoria',
    descripcion: 'Registro histórico de eventos de seguridad',
    contenido: [
      { tipo: 'subtitulo', valor: 'Bitácora de Auditoría de Seguridad' },
      { tipo: 'pasos', items: [
        'Haga clic en "Auditoría" en la barra lateral.',
        'Visualice el registro histórico e inmutable de eventos:'
      ]},
      { tipo: 'tabla', encabezados: ['Columna', 'Descripción'],
        filas: [
          ['Timestamp', 'Fecha y hora exacta del evento'],
          ['Nivel', 'Clasificación: INFO, WARN, SUCCESS, INCIDENTE, ASIGNACION'],
          ['Evento', 'Descripción detallada de la acción y usuario responsable'],
          ['Actor / IP', 'Correo del usuario y dirección IP desde la que se ejecutó']
        ]
      },
      { tipo: 'subtitulo', valor: 'Exportar Auditoría' },
      { tipo: 'texto', valor: 'Haga clic en "Exportar CSV" para descargar el registro completo como archivo CSV.' },
      { tipo: 'nota', variante: 'warning', valor: 'Los registros de auditoría son inmutables. No pueden ser editados ni eliminados por ningún usuario.' }
    ]
  },
  {
    id: 'infraestructura',
          ...
          <div className="flex items-center gap-3 mt-5 max-w-lg">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-3 text-sm bg-white/10 border border-white/20 rounded-lg outline-none focus:ring-2 focus:ring-white/30 placeholder:text-blue-300/60 text-white transition-all"
              />
            </div>
            {searchTerm && (
