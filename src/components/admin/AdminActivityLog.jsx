...
export default function AdminActivityLog({ userName, logs = [] }) {
  ...
  const exportPDF = async () => {
    ...
    const rows = filtered.map(log => [
      new Date(log.aud_time).toLocaleString('es-ES', { year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' }),
      { content: log.aud_tipo, styles: { fontStyle: 'bold' } },
      log.aud_desc,
      `${log.tm_usuar?.usu_crro || 'Sistema'}${log.aud_ip ? ` · ${log.aud_ip}` : ''}`
    ])
    await createStyledPdf('Auditoría de Cambios', `auditoria_${new Date().toISOString().split('T')[0]}.pdf`, columns, rows, {
        ...
        <div className="flex flex-col sm:flex-row gap-3 sm:items-center">
          ...
          <div className="flex flex-col sm:flex-row gap-1 sm:gap-2 items-stretch sm:items-center w-full sm:w-auto">
            <div className="flex items-center gap-1 sm:gap-2">
              ...
              <input type="date" value={dateFrom} onChange={e => { setDateFrom(e.target.value); setPage(0) }} className="flex-1 sm:flex-none px-2 py-2 text-xs sm:text-sm bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-lg outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
            <div className="flex items-center gap-1 sm:gap-2">
              <label className="text-xs text-slate-500 shrink-0">Hasta</label>
              <input type="date" value={dateTo} onChange={e => { setDateTo(e.target.value); setPage(0) }} className="flex-1 sm:flex-none px-2 py-2 text-xs sm:text-sm bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-lg outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
          </div>
          <div className="flex gap-2">
            ...
            <button onClick={exportPDF} className="flex items-center gap-2 px-4 py-2 text-sm font-semibold text-rose-600 bg-rose-50 hover:bg-rose-100 dark:bg-rose-900/30 dark:text-rose-400 dark:hover:bg-rose-900/50 rounded-lg transition-colors shrink-0">
