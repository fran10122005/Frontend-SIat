...
export default function SpecialistGlobalView({
  ...
  const agendaFiltrada = useMemo(() => {
    return agendaHoy.filter((cita) => {
      ...
      if (searchAgenda && !cita.paciente.toLowerCase().includes(q))
        return false;
  ...
  const hasAgendaFilters =
    searchAgenda || filterEstado !== "TODOS" || filterTipo !== "TODOS";
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            <div className="relative w-full sm:flex-1 sm:min-w-[160px]">
              ...
              <input
                ...
                className="w-full pl-3 pr-8 py-1.5 text-xs bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-lg outline-none focus:ring-2 focus:ring-blue-500"
              />
            </div>
            <select
              value={filterEstado}
            ...
            {hasAgendaFilters && (
              <button
                ...
                className="flex items-center gap-1 px-2 py-1.5 text-xs font-semibold text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg transition-colors shrink-0"
              >
          ...
          agendaHoy.length === 0 ? (
            ...
            <div className="flex-1 overflow-y-auto pr-2 space-y-3">
              {agendaFiltrada.map((cita) => (
                <div
                  key={cita.id}
                  className="p-4 rounded-xl border border-slate-100 dark:border-slate-700/50 bg-slate-50 dark:bg-slate-800/50 hover:border-indigo-300 dark:hover:border-indigo-500/50 transition-colors group flex items-center justify-between"
                >
                  <div>
                  ...
                  <div className="flex flex-col gap-2">
                    <button
                      ...
                      title="Ir a paciente"
                      className="action-icon-btn text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 opacity-0 group-hover:opacity-100"
                    >
                    ...
                    {cita.estado !== "Completada" && (
                      <button
                        ...
                        className="action-icon-btn text-slate-500 hover:text-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-900/30 opacity-0 group-hover:opacity-100"
                      >
