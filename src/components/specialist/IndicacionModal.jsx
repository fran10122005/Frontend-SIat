        ...
        <form onSubmit={handleIndicacionSubmit} className="p-6 space-y-4">
          <div>
            ...
            <p className="text-[10px] text-slate-500 mt-2">
              Esta nota será visible inmediatamente para el representante de{" "}
              {activeChild?.nin_nomb || "su paciente"}.
            </p>
          </div>
          <div className="flex justify-end gap-3 pt-4 border-t border-slate-100 dark:border-slate-800">
            <button
              type="button"
              onClick={() => setShowIndicacionModal(false)}
              className="px-4 py-2 font-semibold text-xs text-slate-600 dark:text-slate-350 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg"
            >
              Cancelar
            </button>
            <button
              type="submit"
              className="px-4 py-2 font-semibold text-xs bg-blue-600 text-white hover:bg-blue-700 rounded-lg shadow-sm"
            >
              Guardar y Enviar
            </button>
          </div>
