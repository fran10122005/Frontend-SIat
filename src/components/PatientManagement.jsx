...
export default function PatientManagement() {
  ...
  const filteredNinos = useMemo(() => {
    return listaNinos.filter(nino => {
      ...
      const matchesSearch = fullName.includes(debouncedSearch.toLowerCase()) || nino.nom_nino.toLowerCase().includes(debouncedSearch.toLowerCase())
      const matchesNivel = filterNivel === 'Todos' || nino.niv_desa === filterNivel
      const matchesEstado = filterEstado === 'Todos' || nino.est_disp === filterEstado
              ...
              <div className="flex flex-col sm:flex-row w-full md:w-auto gap-4">
                <div className="relative">
                  ...
                  <input 
                    ...
                    className="w-full sm:w-64 pl-10 pr-4 py-2 border border-gray-300 dark:border-slate-600 rounded-lg bg-gray-50 dark:bg-slate-700 text-gray-800 dark:text-gray-200 focus:outline-none focus:ring-2 focus:ring-brand-500 transition-all"
                    value={searchTerm}
                ...
                {(searchTerm || filterNivel !== 'Todos' || filterEstado !== 'Todos') && (
                  <button onClick={() => { setSearchTerm(''); setFilterNivel('Todos'); setFilterEstado('Todos') }} className="px-3 py-2 text-xs font-semibold text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg transition-colors shrink-0">
                    Limpiar
                  </button>
                )}
              </div>
