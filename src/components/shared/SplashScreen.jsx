...
export default function SplashScreen() {
  ...
  return (
    <div className="fixed inset-0 z-[9999] bg-gradient-to-br from-[#F4F7F9] to-blue-50 dark:from-slate-900 dark:to-slate-950 flex items-center justify-center transition-opacity duration-700">
      <div className="flex flex-col items-center justify-center gap-4 px-4">
        <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-full bg-white dark:bg-slate-800 shadow-xl flex items-center justify-center animate-pulse">
          <img
            src={logo}
            alt="SIAT-TEA"
        ...
        <div className="flex gap-1.5">
          {[0, 1, 2].map((i) => (
            <span
              key={i}
              className="w-2.5 h-2.5 sm:w-3 sm:h-3 bg-blue-600 dark:bg-blue-400 rounded-full animate-bounce"
              style={{ animationDelay: `${i * 150}ms` }}
            />
          ))}
        </div>

        <div className="flex items-center gap-2.5 px-4 sm:px-5 py-3 bg-white dark:bg-slate-800 rounded-2xl shadow-lg border border-slate-200 dark:border-slate-700 animate-in fade-in slide-in-from-bottom-2 duration-500 max-w-sm">
          <IconoActual className="w-4 h-4 sm:w-5 sm:h-5 text-blue-600 dark:text-blue-400 shrink-0" />
