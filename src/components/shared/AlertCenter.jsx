...
import { HeartPulse, Activity, Zap, Filter, Inbox } from 'lucide-react'
        ...
        <div className="relative mt-4">
          ...
          {sortedDates.length === 0 ? (
            <div className="py-16 text-center">
              <div className="inline-flex items-center justify-center w-12 h-12 rounded-full bg-gray-100 dark:bg-slate-800 mb-3 text-gray-400">
                <Inbox className="w-6 h-6" />
              </div>
              <h3 className="text-base font-medium text-gray-800 dark:text-gray-200">No hay alertas registradas</h3>
              <p className="text-gray-500 dark:text-gray-400 mt-1 text-sm">No se encontraron incidentes para los filtros seleccionados.</p>
            </div>
          ) : (
            <div className="space-y-8 before:absolute before:inset-0 before:ml-3.5 before:-translate-x-px before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-gray-200 dark:before:via-slate-700 before:to-transparent">
              {sortedDates.map((date) => (
                <div key={date} className="relative">
                  {/* Date Separator */}
                  <div className="flex items-center justify-start mb-4 relative z-10 ml-1">
                    <span className="bg-white dark:bg-slate-800 text-brand-700 dark:text-blue-400 font-semibold text-xs px-3 py-1 rounded-full border border-gray-200 dark:border-slate-600 shadow-sm">
                      {formatDate(date)}
                    </span>
                  </div>
                  ...
                  <div className="space-y-4">
                    {groupedAlerts[date].map((alert) => (
                      <div key={alert.id_alert} className="relative flex items-start gap-3 group">
                        
                        {/* Icon Marker pequeño */}
                        <div className="flex items-center justify-center w-7 h-7 rounded-full border-2 border-white dark:border-slate-900 bg-amber-100 text-amber-500 shadow shrink-0 z-10 mt-0.5">
                          <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg>
                        </div>

                        {/* Card Body reducido */}
                        <div className="w-full p-3 bg-white dark:bg-slate-800 rounded-lg border border-gray-200 dark:border-slate-700 shadow-sm">
                          <div className="flex justify-between items-start mb-2">
                            <div className="flex items-center gap-2">
                              <span className="text-sm font-bold text-gray-900 dark:text-white">
                                {formatTime(alert.fec_hora)}
                              </span>
                              <span className={`px-2 py-0.5 text-[10px] font-semibold rounded border uppercase tracking-wide ${
                            ...
                            <span className="text-[10px] font-mono text-gray-400 dark:text-gray-500">#{alert.id_alert}</span>
                          </div>
                          
                          <div className="flex flex-wrap gap-2 text-[11px] font-semibold text-slate-500 dark:text-slate-400 mb-2">
                            <span className="flex items-center gap-1"><HeartPulse className="w-3 h-3 text-red-500"/> {alert.bpm_max || 115} BPM</span>
                            <span className="flex items-center gap-1"><Activity className="w-3 h-3 text-indigo-500"/> {alert.mov_max || 2.4} G</span>
                            <span className="flex items-center gap-1"><Zap className="w-3 h-3 text-amber-500"/> Estrés: {alert.stress_index || 75}%</span>
                          </div>

                          <div className="pt-2 border-t border-gray-100 dark:border-slate-700">
                            {alert.fue_efec === null ? (
                              <div className="flex items-center gap-2">
                                <span className="text-[11px] text-gray-500 dark:text-gray-400 shrink-0">¿Efectiva?</span>
                                <button 
                                  onClick={() => evaluateAlert(alert.id_alert, true)}
                                  className="px-2.5 py-1 bg-green-600 hover:bg-green-700 text-white text-[10px] font-semibold rounded transition-colors"
                                >
                                  Sí
                                </button>
                                <button 
                                  onClick={() => evaluateAlert(alert.id_alert, false)}
                                  className="px-2.5 py-1 bg-white dark:bg-slate-800 hover:bg-red-50 dark:hover:bg-red-900/20 text-red-600 dark:text-red-400 border border-red-200 dark:border-red-800 text-[10px] font-semibold rounded transition-colors"
                                >
                                  No
                                </button>
                              </div>
                            ) : (
                              <div className={`flex items-center gap-1.5 py-1 px-2 rounded text-[11px] font-semibold ${
                                alert.fue_efec 
                                ...
                                {alert.fue_efec ? 'Exitosa' : 'No efectiva'}
                              </div>
                            )}
                          </div>
                        </div>
